...
function legalMention(profile: Profile, docType: string) {
  ...
  if (docType === 'invoice') parts.push('Pénalités de retard : 3× taux légal — Indemnité forfaitaire : 40€');
  return parts.join(' • ');
}
...
export function generateInvoiceHtml(invoice: Invoice, profile?: Profile | null): string {
  ...
  const paymentBtn = invoice.stripe_payment_url ? `<div style="text-align:center;margin:24px 0"><a href="${invoice.stripe_payment_url}" style="display:inline-block;background:${accent};color:#fff;font-weight:700;font-size:14px;padding:14px 36px;border-radius:8px;text-decoration:none">Payer ${f(invoice.total)} en ligne</a></div>` : '';

  return `<!DOCTYPE html><html lang="${p.language||'fr'}"><head><meta charset="UTF-8"/><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:13px;color:#1a1a2e;background:#fff}@page{margin:0;size:A4}</style></head><body>
...
${bankBlock}${paymentBtn}${signatureBlock}
