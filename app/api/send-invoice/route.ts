...
function buildEmailHtml(invoice: any, profile: any, senderName: string): string {
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">
        ...
        <tr><td style="background:#ffffff;padding:32px 40px">
          ...
          <p style="font-size:14px;color:#6b7280;margin:0 0 28px;line-height:1.6">
            Veuillez trouver ci-joint votre ${docLabel.toLowerCase()} <strong style="color:#111827">${invoice.number}</strong> de la part de <strong style="color:#111827">${senderName}</strong>.
          </p>
...
export async function POST(req: NextRequest) {
  try {
    const { invoiceId, email, profile, isReminder } = await req.json();
    if (!invoiceId || !email) return NextResponse.json({ error: 'invoiceId and email required' }, { status: 400 });

    const supabase = createAdminClient();
    const { data: invoice, error } = await supabase.from('invoices').select('*, client:clients(*)').eq('id', invoiceId).single();
    if (error || !invoice) return NextResponse.json({ error: 'Invoice not found' }, { status: 404 });

    const BREVO_API_KEY = process.env.BREVO_API_KEY;
    if (!BREVO_API_KEY) return NextResponse.json({ error: 'Email service not configured' }, { status: 500 });

    const senderName = profile?.company_name || 'Factu.me';
    const senderEmail = profile?.email || process.env.BREVO_SENDER_EMAIL || 'noreply@factu.me';
    const pdfHtml = generateInvoiceHtml(invoice, profile);

    const subjectPrefix = isReminder ? '[Rappel] ' : '';
    const subject = `${subjectPrefix}${invoice.number} – ${senderName}`;
    ...
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        to: [{ email }],
        subject,
        htmlContent: buildEmailHtml(invoice, profile, senderName),
        attachment: [{ name: `${invoice.number}.html`, content: Buffer.from(pdfHtml).toString('base64') }],
      }),
    });
