...
export default function CDIListPage() {
  const { profile } = useAuthStore();
  const [contracts, setContracts] = useState<CDIContract[]>([]);
  ...
  const loadContracts = async () => {
    ...
    try {
      ...
      if (data) {
        setContracts(data as CDIContract[]);
      }
    } catch (error) {
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Gérez vos contrats à durée indéterminée
              </p>
            </div>
            <Link href="/contracts/cdi">
              <button className="px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center gap-2">
          ...
          {loading ? (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                Aucun contrat CDI trouvé
              </h3>
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                {searchQuery || statusFilter !== 'all'
                  ? 'Essayez de modifier vos filtres de recherche'
                  : 'Créez votre premier contrat CDI'}
              </p>
              {!searchQuery && statusFilter === 'all' && (
                <Link href="/contracts/cdi">
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                      <span className="flex items-center gap-1">
                        ...
                        Depuis le {new Date(contract.contract_start_date).toLocaleDateString('fr-FR')}
                      </span>
                      <span className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <Link href={`/contracts/cdi?edit=${contract.id}`}>
