...
export default function OtherListPage() {
  ...
  const [contracts, setContracts] = useState<OtherContract[]>([]);
  ...
  const loadContracts = async () => {
    ...
    try {
      ...
      if (data) {
        setContracts(data as OtherContract[]);
      }
    } catch (error) {
  ...
  const filteredContracts = contracts.filter(contract => {
    ...
    const matchesCategory = categoryFilter === 'all' || contract.contract_category === categoryFilter;
  ...
  const categoryLabels: Record<string, string> = {
    ...
    cui_cie: 'CUI CIE',
    cui_cae: 'CUI CAE',
    domicile: 'Travail à domicile',
    other: 'Autre'
  };

  const categoryColors: Record<string, string> = {
    stage: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400',
    apprenticeship: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400',
    professionalization: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400',
    portage: 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400',
    interim: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    freelance: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
    cui_cie: 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400',
    cui_cae: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
    domicile: 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400',
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Gérez vos stages, freelances, intérim, alternance...
              </p>
            </div>
            <Link href="/contracts/other">
              <button className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center gap-2">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative">
              <select
                ...
                className="px-4 py-2 rounded-xl border-2 border-gray-200 dark:border-white/10 bg-white/50 dark:bg-slate-800/50 focus:border-primary/50 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm appearance-none"
              >
                ...
                <option value="cui_cie">CUI CIE</option>
                <option value="cui_cae">CUI CAE</option>
              </select>
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${categoryColors[contract.contract_category]}`}>
                        {categoryLabels[contract.contract_category] || contract.contract_category}
                      </span>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-2 mb-1">
                      ...
                      {contract.contract_title && ` · ${contract.contract_title}`}
                    </p>
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400 flex-wrap">
                      ...
                      {contract.salary_amount && (
                        <span className="flex items-center gap-1">
                          {contract.salary_amount?.toFixed(2)} € {contract.salary_frequency === 'monthly' ? '/mois' : contract.salary_frequency === 'weekly' ? '/semaine' : contract.salary_frequency === 'hourly' ? '/heure' : ''}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link href={`/contracts/other?edit=${contract.id}`}>
