...
export default function CDIContractPage() {
  ...
  const handleTextSubmit = async () => {
    ...
    try {
      const response = await fetch('/api/process-text-contract', {
        ...
        body: JSON.stringify({ text: textInput, contract_type: 'cdi' }),
      });
      if (!response.ok) throw new Error('Erreur lors de l\'analyse');
      const result = await response.json();
      if (result.parsed) {
  ...
  const generateContract = async () => {
    const html = generateCDITemplate({ ...formData, contractType: 'cdi' as const });
    setContractHtml(html);
    setStep('preview');
    // Load PDF preview in background
    loadPdfPreview();
  };
  ...
  const loadPdfPreview = async () => {
    ...
    if (hasMissing) return; // Don't attempt preview if fields missing
    ...
    try {
      const response = await fetch('/api/contracts/pdf', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contract: { ...formData, contractType: 'cdi' as const, employeeNationality: formData.employeeNationality || 'Française', employerTitle: formData.employerTitle || 'Gérant', workSchedule: formData.workSchedule || '35h hebdomadaires', salaryFrequency: formData.salaryFrequency || 'monthly' }
        }),
      });
      if (response.ok) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          employee_qualification: formData.employeeQualification || null,
          ...
          contract_classification: formData.contractClassification || null,
            ...
            <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white/30 dark:border-white/10 shadow-lg rounded-3xl p-6">
              ...
              <div className="flex flex-col gap-2">
                <textarea
                  ...
                  placeholder="Décrivez le salarié et le contrat en texte libre... Ex : 'Jean Martin, né le 10/05/1988, ingénieur logiciel, CDI à compter du 1er septembre 2024. Salaire 4 200 € brut/mois, statut cadre.'"
                  rows={4}
            ...
            <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white/30 dark:border-white/10 shadow-lg rounded-3xl p-8">
              ...
              {!pdfPreviewLoading && pdfPreviewUrl ? (
                <iframe
                  ...
                  style={{ height: '600px' }}
                  title="Aperçu PDF du contrat CDI"
                />
              ) : !pdfPreviewLoading && (
                <iframe
                  srcDoc={contractHtml}
