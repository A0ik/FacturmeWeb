...
export default function CDDContractPage() {
  ...
  const saveContract = async () => {
    ...
    try {
      ...
      const cleanHtml = generateCDDTemplate({ ...cleanFormData, contractType: 'cdd' as const });
  ...
  const downloadPDF = async () => {
    ...
    try {
      // Utiliser l'API qui retourne le fichier PDF directement
      const response = await fetch('/api/contracts/pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      a.href = url;
      a.download = `Contrat_CDD_${formData.employeeLastName || 'Salarie'}_${new Date().toISOString().split('T')[0]}.pdf`;
