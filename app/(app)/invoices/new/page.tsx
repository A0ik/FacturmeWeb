...
export default function NewInvoicePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push(`/invoices/${inv.id}`), 600);
    } catch (e: any) {
        ...
        {DOC_TYPES.map((dt) => {
          ...
          return (
            <button
              ...
              className={cn(
                'relative flex flex-col items-center gap-2 py-4 px-3 rounded-2xl border-2 text-center transition-all duration-200',
                active
                  ? `${dt.border} bg-white shadow-md shadow-black/5`
                  : 'border-gray-100 bg-white text-gray-400 hover:border-gray-200 hover:text-gray-700',
              )}
            >
              <div className={cn(
              ...
              <div>
                <p className={cn('text-sm font-bold transition-colors', active ? 'text-gray-900' : 'text-gray-500')}>{dt.label}</p>
                <p className={cn('text-[10px] mt-0.5 hidden sm:block transition-colors', active ? 'text-gray-400' : 'text-gray-300')}>{dt.description}</p>
              </div>
              {active && (
          ...
          {!sub.canUseVoice && (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-amber-600 mt-0.5">Dictez vos factures à voix haute, l'IA les remplit automatiquement →</p>
              </div>
            </Link>
          )}
            ...
            <div className="space-y-1 text-center">
              ...
              {!recording && !loading && (
                <p className="text-xs text-gray-300">
                  Ex: "Facture pour Dupont SA, développement site web, 5 jours à 500€, TVA 20%"
                </p>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-3">
              ...
              <div className="p-4">
                  ...
                  {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 z-20 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden">
                      {suggestions.map((c) => (
                        ...
                        >
                          <div className="w-7 h-7 rounded-lg bg-primary-light flex items-center justify-center text-primary text-xs font-bold flex-shrink-0">
                ...
                {clientId && (
                  <p className="text-xs text-primary flex items-center gap-1 mt-2">
                    <CheckCircle2 size={11} /> Client associé — infos bancaires incluses automatiquement
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="px-4 pb-3 flex gap-2">
                {[15, 30, 45, 60].map((days) => (
                  <button
                    ...
                    className="text-xs px-2.5 py-1 rounded-lg bg-gray-100 text-gray-500 hover:bg-primary-light hover:text-primary font-semibold transition-colors"
                  >
                    J+{days}
                  </button>
                ))}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-50 bg-gray-50/50">
                <AlignLeft size={14} className="text-gray-400" />
                <h3 className="text-sm font-bold text-gray-700">Notes & conditions de paiement</h3>
              </div>
              <div className="p-4">
                <Textarea
                  placeholder={profile?.payment_terms || 'Payable par virement bancaire sous 30 jours à compter de la date de facture...'}
                  value={notes}
                ...
                {profile?.payment_terms && !notes && (
                  ...
                  >
                    Utiliser mes conditions de paiement habituelles
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="lg:sticky lg:top-6 space-y-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-2.5 shadow-sm">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-semibold text-gray-700 capitalize">{docConfig.label}</span>
                </div>
                <div className="flex justify-between text-xs">
              ...
              <button
                ...
                className={cn(
                  ...
                  (saving || success) && 'cursor-not-allowed',
                )}
              >
                {saving ? (
                  <>
              ...
              {sub.isAtLimit && (
                <Link
                  href="/paywall"
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold hover:bg-amber-100 transition-colors"
                >
